...
const Themes wavrTheme::availableThemes(void) {
...
const ThemeData wavrTheme::loadTheme(const QString &path) {
    QFile file;
    ThemeData themeData;
    ...
    if(!file.open(QIODevice::ReadOnly))
        return loadTheme(defTheme);
    themeData.inMsg = "<div class='_wavr_chatdiv'>" + QString(file.readAll().constData()) + "</div>";
    file.close();
    ...
    if(!file.open(QIODevice::ReadOnly))
        return loadTheme(defTheme);
    themeData.outMsg = "<div class='_wavr_chatdiv'>" + QString(file.readAll().constData()) + "</div>";
    file.close();
    ...
    if(!file.open(QIODevice::ReadOnly))
        return loadTheme(defTheme);
    themeData.pubMsg = "<div class='_wavr_publicdiv'>" + QString(file.readAll().constData()) + "</div>";
    file.close();
    ...
    themeData.sysMsg = "<div class='_wavr_sysdiv'>" + sys + "</div>";
    themeData.stateMsg = "<div class='_wavr_statediv'>" + sys + "</div>";
    file.close();
    ...
    if(!file.open(QIODevice::ReadOnly))
        return loadTheme(defTheme);
    themeData.reqMsg = "<div class='_wavr_reqdiv'>" + QString(file.readAll().constData()) + "</div>";
    file.close();
